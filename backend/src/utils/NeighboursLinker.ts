...
export class NeighboursLinker {
  ...
  static linkNeighboursToGraduatedStudents(
    allGraduatedStudents: GraduatedStudent[]
  ): GraduatedStudent[] {
    allGraduatedStudents.forEach((student) => {
      const neighbourWords = student
        .getNeighboursString()
        .toLowerCase()
        .split(/[\s,;:.!?]+/);
      const normalizedNeighbourWords = neighbourWords.map((word) =>
        StringNormalizer.normalizeString(word)
      );

      // Case 1 : Matching on fullname with max 2 errors in the combination
      type firstCaseCandidateWithDistance = {
        candidate: GraduatedStudent;
        distance: number;
      };
      ...
      for (let i = 0; i < normalizedNeighbourWords.length - 1; i++) {
        const combinedNormalizedWords = StringNormalizer.normalizeString(
          normalizedNeighbourWords[i] + " " + normalizedNeighbourWords[i + 1]
        );
        for (const candidate of allGraduatedStudents) {
          const firstNameLastName = StringNormalizer.normalizeString(
            candidate.getFirstName() + " " + candidate.getLastName()
          );
          const lastNameFirstName = StringNormalizer.normalizeString(
            candidate.getLastName() + " " + candidate.getFirstName()
          );
          const distance1 = levenshtein(
            firstNameLastName,
            combinedNormalizedWords
          );
          const distance2 = levenshtein(
            lastNameFirstName,
            combinedNormalizedWords
          );
          const currentDistance = Math.min(distance1, distance2);
          if (
            currentDistance <= this.levenshteinThresholdFullName &&
            currentDistance <= firstCaseBestDistance
          ) {
      ...
      for (const { candidate, distance } of firstCaseCandidates) {
        if (
          distance === 0 &&
          !student.isNeighboursAlreadyPresent(candidate) &&
          !student.isSameStudent(candidate)
        ) {
          student.addNeighbour(candidate);
        } else if (
          distance === firstCaseBestDistance &&
          !student.isPotentialNeighboursAlreadyPresent(candidate) &&
          !student.isSameStudent(candidate)
        ) {
      ...
      type secondCaseCandidateWithDistance = {
        candidate: GraduatedStudent;
        distance: number;
      };
      ...
      for (let i = 0; i < normalizedNeighbourWords.length; i++) {
        for (const candidate of allGraduatedStudents) {
          const lastName = StringNormalizer.normalizeString(
            candidate.getLastName()
          );
          const distance = levenshtein(lastName, normalizedNeighbourWords[i]);
          if (
            distance <= this.levenshteinThresholdLastName &&
            distance <= secondCaseBestDistance
          ) {
  ...
  static async addNeighbour(
    allGraduatedStudents: GraduatedStudent[],
    id_student: number,
    id_neighbour: number
  ): Promise<void> {
    const student = allGraduatedStudents.find((s) => s.getId() == id_student);
    const neighbour = allGraduatedStudents.find(
      (n) => n.getId() == id_neighbour
    );
  ...
  static async removeNeighbour(
    allGraduatedStudents: GraduatedStudent[],
    id_student: number,
    id_neighbour: number
  ): Promise<void> {
    const student = allGraduatedStudents.find((s) => s.getId() == id_student);
    student!.deleteNeighbour(id_neighbour);
  }
}
